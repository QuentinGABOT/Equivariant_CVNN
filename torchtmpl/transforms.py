...
class ZeroPhaseTransform:
    ...
    def __call__(self, tensor) -> torch.Tensor:
        new_tensor = []
        for idx, ch in enumerate(tensor):
            amplitude = torch.as_tensor(ch)
            phase = torch.rand_like(amplitude, dtype=torch.float64) * 2 * np.pi
            new_tensor.append(amplitude * torch.exp(1j * phase))
        return torch.as_tensor(np.stack(new_tensor), dtype=torch.complex64)
...
class CreateTensor:
    ...
    def __call__(self, element) -> torch.Tensor:
        if len(element.shape) == 2:
            ...
            raise ValueError("Element should be a 2D or 3D numpy array or tensor")
