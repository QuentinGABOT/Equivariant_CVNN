"""
Experiment implementation for the Generation task.

This module provides the GenerationExperiment class that handles
training, evaluation, and visualization for generative modeling tasks.
"""
...
from cvnn.inference import (
    ...
    interpolate_from_dataloader,
    sample_from_model_distribution,
    inference_on_dataloader,
    fit_gmm_on_dataloader,
    perform_phase_twist,
    get_all_latents,
)
...
class GenerationExperiment(BaseExperiment):
    ...
    generative modeling tasks using Variational AutoEncoder architecture.
    """

    def build_model(self) -> torch.nn.Module:
        """Build and return the generation model using model registry.

        Returns:
            torch.nn.Module: The generation model (AutoEncoder).
        """
        return build_model_from_config(self.cfg, task="generation")
    ...
    def evaluate(self) -> Dict[str, Any]:
        ...
        sample_gmm = self.cfg["model"].get("sample_gmm")
        if sample_gmm:
            logger.info("Extracting latents for GMM fitting...")
            fit_gmm_on_dataloader(
                model=self.model,
        ...
        torch.save(self.model.state_dict(), Path(self.logdir) / "model_with_gmm.pt")
        ...
        if self.cfg["data"].get("has_labels"):
            # 2. Linear Probing (Semantic Utility)
            probing_acc = compute_linear_probing(
                model=self.model,
            ...
            if probing_acc is not None:
                self.metrics["metrics"]["linear_probing_accuracy"] = probing_acc
        ...
        if self.wandb_log and hasattr(wandb, "run") and wandb.run:
            ...
            wandb.log({f"generation/{k}": v for k, v in self.metrics["metrics"].items()})
    ...
    def visualize(self) -> None:
        ...
        if self.cfg["data"].get(
            ...
            save_path = Path(self.logdir) / "dataset_split_visualization.pdf"
            ...
            save_path = Path(self.logdir) / "reconstruction_error_analysis.pdf"
            fig.savefig(save_path, dpi=300, bbox_inches="tight")
        ...
        if self.cfg.get("mode") in ("full", "train", "retrain"):
            ...
            save_path = Path(self.logdir) / "loss_curve.pdf"
            fig.savefig(save_path, dpi=300, bbox_inches="tight")
        ...
        if self.cfg["data"].get("type").lower() == "polsar":
            ...
            save_path = Path(self.logdir) / "pauli_decomposition.pdf"
            fig.savefig(save_path, dpi=300, bbox_inches="tight")
            ...
            save_path = Path(self.logdir) / "krogager_decomposition.pdf"
            fig.savefig(save_path, dpi=300, bbox_inches="tight")
            ...
            save_path = Path(self.logdir) / "h_alpha_decomposition.pdf"
            fig.savefig(save_path, dpi=300, bbox_inches="tight")
            ...
            save_path = Path(self.logdir) / "h_alpha_plane.pdf"
            fig.savefig(save_path, dpi=300, bbox_inches="tight")
            ...
            save_path = Path(self.logdir) / "h_alpha_classification_metrics.pdf"
            fig.savefig(save_path, dpi=300, bbox_inches="tight")
            ...
            save_path = Path(self.logdir) / "cameron_decomposition.pdf"
            fig.savefig(save_path, dpi=300, bbox_inches="tight")
            ...
            save_path = Path(self.logdir) / "cameron_classification_metrics.pdf"
            fig.savefig(save_path, dpi=300, bbox_inches="tight")
        ...
        latents, labels = get_all_latents(
            model=self.model,
        ...
        save_path = Path(self.logdir) / "latent_space_visualization.pdf"
        fig.savefig(save_path, dpi=300, bbox_inches="tight")
        ...
        save_path = Path(self.logdir) / "latent_space_umap_visualization.pdf"
        fig.savefig(save_path, dpi=300, bbox_inches="tight")
        logger.info(f"Saved latent space UMAP visualization to {save_path}")
        if self.wandb_log and hasattr(wandb, "run") and wandb.run:
        ...
        save_path = Path(self.logdir) / "reconstructions.pdf"
        fig.savefig(save_path, dpi=300, bbox_inches="tight")
        ...
        imgs, _, _ = interpolate_from_dataloader(
            model=self.model, dataloader=eval_loader, num_samples=5, mode="lerp", device=self.device
        )
        imgs = np.array([revert_transforms(img, self.cfg) for img in imgs])
        fig = plot_interpolation_grid(images_grid=imgs, 
                                      dataset_type=self.cfg["data"].get("type").lower(),
                                      show_spectrum=False,
        )
        save_path = Path(self.logdir) / "latent_space_interpolations_grid.pdf"
        fig.savefig(save_path, dpi=300, bbox_inches="tight")
        logger.info(f"Saved latent space interpolations grid to {save_path}")
        if self.wandb_log and hasattr(wandb, "run") and wandb.run:
            wandb.log({"latent_space_interpolations_grid": wandb.Image(fig)})
        plt.close(fig)

        ### Random latent space interpolations ###

        outputs = sample_from_model_distribution(model=self.model, 
                                                          num_samples=10,
        )
        outputs = revert_transforms(outputs, self.cfg)
        fig = plot_random_interpolations(
            outputs=outputs,
            dataset_type=self.cfg["data"].get("type").lower(),
            num_samples=10,
            show_spectrum=True,
        )
        save_path = Path(self.logdir) / "random_latent_space_interpolations.pdf"
        fig.savefig(save_path, dpi=300, bbox_inches="tight")
        logger.info(f"Saved random latent space interpolations to {save_path}")
        if self.wandb_log and hasattr(wandb, "run") and wandb.run:
            wandb.log({"random_latent_space_interpolations": wandb.Image(fig)})
        plt.close(fig)

        # ### Phase Twist Experiment ###
        batch = next(iter(eval_loader))
        input_sample, twist_frames = perform_phase_twist(
            model=self.model, 
            batch=batch,
            steps=8,
            device=self.device
        ...
        input_sample = revert_transforms(input_sample, self.cfg)
        twist_frames = revert_transforms(twist_frames, self.cfg)
        fig = plot_phase_twist_experiment(
            original_image=input_sample.squeeze(),
            twisted_frames=twist_frames,
            dataset_type=self.cfg["data"].get("type").lower()
        )
        save_path = Path(self.logdir) / "phase_twist_demonstration.pdf"
        fig.savefig(save_path, dpi=300, bbox_inches="tight")
        logger.info(f"Saved phase twist demonstration to {save_path}")
        if self.wandb_log and hasattr(wandb, "run") and wandb.run:
            wandb.log({"phase_twist_demonstration": wandb.Image(fig)})
