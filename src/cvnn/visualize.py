...
def plot_random_interpolations(
    outputs,
    dataset_type: str,
    num_samples: int = 5,
    show_spectrum: bool = False
):
    """Plot random interpolation samples from generated outputs.
    
    Args:
        outputs: Generated outputs from the model (numpy array or torch tensor) [B, C, H, W]
        dataset_type: Type of dataset ("polsar", "sar", "grayscale")
        num_samples: Number of random samples to plot
        show_spectrum: If True, adds C rows (one per channel) displaying the Fourier Amplitude.
        
    Returns:
        Matplotlib figure object containing the random interpolation samples. 
    """
    ...
    figsize_h = 3.5 * nrows
    figsize_w = 3.5 * num_cols

    fig, axes = plt.subplots(nrows, num_cols, figsize=(figsize_w, figsize_h), 
                             squeeze=False, constrained_layout=True)

    for s in range(num_samples):
        img_tensor = outputs[s] # (C, H, W)
        
        vis_img, cmap, origin, _ = _display_img(img_tensor, dataset_type)
        
        ax_spatial = axes[0][s]
        ax_spatial.imshow(vis_img, cmap=cmap, origin=origin)
        
        if s == 0:
            ax_spatial.set_ylabel("Spatial Domain", fontsize=10, fontweight='bold')
        ...
        if show_spectrum:
            amp_list, _ = plot_fourier_transform_amplitude_phase(img_tensor)

            for c in range(C):
                row_idx = 1 + c
                spec_img = amp_list[c]
                spec_img, _ = equalize(spec_img)
                
                ax_freq = axes[row_idx][s]
                ax_freq.imshow(spec_img, cmap='inferno', origin='upper')
                
                if s == 0:
                    ax_freq.set_ylabel(f"Spectrum\nChannel {c}", fontsize=9)
...
def plot_interpolation_grid(
    images_grid: np.ndarray, 
    dataset_type: str, 
    show_spectrum: bool = False
) -> plt.Figure:
    """Plot a grid of interpolated images.
    
    If show_spectrum is True, interleaves spectral amplitude rows immediately 
    below each spatial row.

    Args:
        images_grid: 5D numpy array (n_rows, n_cols, C, H, W) 
                     or 4D (n_rows, n_cols, H, W) where C is inferred as 1.
        dataset_type: Type of dataset ("polsar", "sar", "grayscale").
        show_spectrum: If True, adds C spectral rows for every spatial row.

    Returns:
        Matplotlib figure object. 
    """
    
    if images_grid.ndim == 4:
        images_grid = images_grid[:, :, np.newaxis, :, :]
        
    n_input_rows, n_cols, C, H, W = images_grid.shape

    rows_per_sample = 1 + C if show_spectrum else 1
    total_plot_rows = n_input_rows * rows_per_sample
    
    figsize_h = 3.0 * total_plot_rows
    figsize_w = 3.0 * n_cols

    fig, axes = plt.subplots(
        total_plot_rows, n_cols, 
    ...
    for i in range(n_input_rows):
        for j in range(n_cols):
            img_tensor = images_grid[i, j] # Shape (C, H, W)

            spatial_row_idx = i * rows_per_sample
            
            vis_img, cmap, origin, _ = _display_img(img_tensor, dataset_type)
            
            ax_spatial = axes[spatial_row_idx][j]
            ax_spatial.imshow(vis_img, cmap=cmap, origin=origin)
            ax_spatial.axis("off")
            
            if j == 0 and show_spectrum:
                ax_spatial.set_ylabel(f"Interp Row {i}\nSpatial", fontsize=9, fontweight='bold')
                ax_spatial.axis("on")
                ax_spatial.set_xticks([])
                ax_spatial.set_yticks([])
                for spine in ax_spatial.spines.values(): spine.set_visible(False)

            if show_spectrum:
                amp_list, _ = plot_fourier_transform_amplitude_phase(img_tensor)
                
                for c in range(C):
                    spectral_row_idx = spatial_row_idx + 1 + c
                    
                    ax_freq = axes[spectral_row_idx][j]
                    spec_img = amp_list[c]
                    spec_img, _ = equalize(spec_img)
                    ...
                    if j == 0:
                        ...
                        ax_freq.set_xticks([])
                        ax_freq.set_yticks([])
                        for spine in ax_freq.spines.values(): spine.set_visible(False)

    return fig

def plot_phase_twist_experiment(
    original_image: np.ndarray,
    twisted_frames: np.ndarray,
    dataset_type: str,
) -> plt.Figure:
    """
    Visualizes the Phase Twist experiment using the unified _display_img helper.
    
    Args:
        original_image: Ground truth image (Numpy)
        twisted_frames: Array of reconstructed frames [Steps, C, H, W] (Numpy)
        dataset_type: Type of dataset ("polsar", "sar", "grayscale")        
    """
    num_frames = twisted_frames.shape[0]
    total_cols = num_frames + 1
    
    fig, axes = plt.subplots(2, total_cols, figsize=(2 * total_cols, 5), constrained_layout=True)
    

    img_disp, cmap, origin, (p1, p2) = _display_img(original_image, dataset_type)
    
    axes[0, 0].imshow(img_disp, cmap=cmap, origin=origin)
    axes[0, 0].set_title("Ground Truth", fontsize=10)
    axes[0, 0].axis("off")
    axes[0, 0].set_ylabel("Magnitude", fontsize=12, fontweight='bold')

    phase_orig = _get_phase_img(original_image, dataset_type)
    axes[1, 0].imshow(phase_orig, cmap="twilight", vmin=-np.pi, vmax=np.pi, origin=origin)
    axes[1, 0].set_ylabel("Phase", fontsize=12, fontweight='bold')
    axes[1, 0].axis("off")

    for i in range(num_frames):
        col_idx = i + 1
        img_raw = twisted_frames[i]

        img_disp, cmap, origin, _ = _display_img(img_raw, dataset_type)
        
        if i == 0:
            title = "Recon ($\\theta=0$)"
        else:
            deg = int(i * 360 / num_frames)
            title = f"$\\theta={deg}^\\circ$"
            
        axes[0, col_idx].imshow(img_disp, cmap=cmap, origin=origin)
        axes[0, col_idx].set_title(title, fontsize=10)
        axes[0, col_idx].axis("off")

        # B. Phase Row
        phase_img = _get_phase_img(img_raw, dataset_type)
        axes[1, col_idx].imshow(phase_img, cmap="twilight", vmin=-np.pi, vmax=np.pi, origin=origin)
        axes[1, col_idx].axis("off")
        
    return fig

def plot_latent_space(latents, labels, method="tsne", classes_names=None):
    """
    Orchestrates extraction and plotting.
    """    
    if np.iscomplexobj(latents):
        print("Complex latents detected. Concatenating [Real, Imag].")
    ...
    if labels is not None:
        ...
        if classes_names is not None:
            ...
            class_names = [classes_names[int(i)] for i in unique_labels]
